            ...
            <div className="container px-4 md:px-6 flex flex-col gap-6 items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {pressData.slice(0, 2).map((press, index) => (
                        <PressReleaseCard locale={locale} slug={press.slug} key={index} title={press.title} description={press.excerpt} image={press.image || "https://placehold.co/600x400"} />
                    ))}
                </div>
            </div>
        </section>
...
const PressReleaseCard = ({ title, description, image, slug, locale }: { title: string; description: string; image: string; slug: string; locale: string }) => {
    const t = useTranslations("HomePage")
    return (
        <div className="bg-[#1B1818] text-white rounded-3xl flex flex-col gap-4 group w-full">
            ...
            <div className="flex flex-col justify-between h-full">
                ...
                <a href={`/${locale}/article/${slug}`} className="relative flex items-center gap-2 group-hover:bg-primary group-hover:text-white transition duration-200 p-8 rounded-b-2xl">
